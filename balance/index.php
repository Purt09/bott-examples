...
$coef = isset($_GET['coef']) ? $_GET['coef'] : null;
...
if ($count === null || $count === '') {
    app_log('error', array('reason' => 'missing_count', 'bot_id' => $bot_id));
    exit('not found count');
}

$bot_user_id = null;
if (isset($_POST['botUser']['id'])) {
    $bot_user_id = $_POST['botUser']['id'];
}
if ($bot_user_id === null || $bot_user_id === '') {
    app_log('error', array('reason' => 'missing_bot_user_id', 'bot_id' => $bot_id, 'count' => $count));
...
if ($result === false) {
    app_log('error', array('reason' => 'bott_api_request_failed', 'recipient_user_id' => $bot_user_id, 'count' => $count));
    exit;
}
