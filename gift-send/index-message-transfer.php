...
if ($telegram_id === null || $telegram_id === '' || !preg_match('/^-?\d+$/', (string) $telegram_id)) {
    app_log_step_error('validate_body', ['reason' => 'missing_recipient_telegram_id', 'user_id' => $user_id, 'message_id' => $message_id]);
    http_response_code(400);
...
app_log_step('prepare_bott_payload', ['method' => 'transferGift', 'user_id' => $user_id, 'message_id' => $message_id]);

$url = 'https://api.bot-t.com/v1/bot/user/send-request?token=' . rawurlencode($token);

app_log_step('bott_api_request', ['method' => 'transferGift', 'user_id' => $user_id, 'message_id' => $message_id]);
...
app_log_step_bott('bott_api_response', $response, ['user_id' => $user_id, 'message_id' => $message_id, 'method' => 'transferGift']);
...
app_log_step('finish', ['ok' => true, 'user_id' => $user_id, 'message_id' => $message_id, 'method' => 'transferGift']);
